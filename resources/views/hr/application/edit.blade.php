                            ...
                            <div class="card-header d-flex align-items-center justify-content-between">
                                ...
                                <div class="d-flex align-items-center">
                                    <div class="d-flex flex-column align-items-end">
                                        ...
                                        <div class="text-success"><i class="fa fa-check"></i>&nbsp;{{ config('constants.hr.status.confirmed.title') }}
                                        </div>
                                        @elseif ($applicationRound->round_status == config('constants.hr.status.rejected.label'))
                                        <div class="text-danger"><i class="fa fa-close"></i>&nbsp;{{ config('constants.hr.status.rejected.title') }}
                                        </div>
                                        @endif
                                    ...
                                    <div class="text-right ml-5 ml-md-0">
                                        <button type="button" id="meet_time" value="{{ $applicationRound->id }}" class="btn-sm btn-primary text-decoration-none finish_interview ">Finish
                                            Interview</button>
                                    </div>
                                    @endif
                                    @endif
                                    <div class="icon-pencil position-relative ml-3 c-pointer" data-toggle="collapse" data-target="#collapse_{{ $loop->iteration }}">
                                ...
                                <div id="collapse_{{ $loop->iteration }}" class="collapse {{ $application->latestApplicationRound->round->isTrialRound() ? ($loop->last ? 'show' : '') : '' }}">
                                    <div class="card-body">
                                        ...
                                        <div class="form-row">
                                            <div class="form-group col-md-4">
                                                <label for="scheduled_date" class="fz-12 leading-none text-secondary w-100p">
                                                    <div>
                                                ...
                                                @if ($applicationRound->scheduled_date)
                                                <input type="date" name="scheduled_date" id="scheduled_date" class="form-control form-control-sm" value="{{ $applicationRound->scheduled_date->format(config('constants.date_format')) }}">
                                                @else
                                            ...
                                            <div class="form-group col-md-4">
                                                <label for="scheduled_time" class="fz-14 leading-none text-secondary w-100p">
                                                    <div>
                                                        <span>Scheduled Time</span>
                                                    </div>
                                                </label>
                                                @if ($applicationRound->scheduled_date)
                                                <input type="Time" name="scheduled_time" id="scheduled_time" class="form-control form-control-sm" value="{{ $applicationRound->scheduled_date->toTimeString() }}">
                                                @endif
                                            </div>
                                            <div class="form-group col-md-4">
                                            ...
                                            <div class="form-group col-md-3 d-flex align-items-end">
                                                <button type="button" class="py-1 mb-0 btn btn-info btn-sm round-submit update-schedule" data-action="schedule-update">Update
                                                    Schedule</button>
                                            </div>
                                            @endif
                                        </div>
                                        ...
                                        <div class="form-row">
                                            <div class="form-group col-md-12">
                                                <button type="button" class="btn btn-theme-fog btn-sm" @click="getApplicationEvaluation({{ $applicationRound->id, $applicationRound->hr_round_id }})">Application
                                                    Evaluation</button>
                                            </div>
                                        </div>
                                        @endif
                                    ...
                                    <div class="card-footer">
                                        <div class="d-flex align-items-center">
                                            ...
                                            <select name="action_type" id="action_type" class="form-control w-50p" v-on:change="onSelectNextRound($event)" data-application-job-rounds="{{ json_encode($application->job->trialRounds) }}">
                                                <option v-for="round in applicationJobRounds" value="round" :data-next-round-id="round.id">
                                            ...
                                            <button type="button" class="btn btn-primary ml-auto" data-toggle="modal" data-target="#round_{{ $applicationRound->id }}">Send
                                                mail</button>
                                            @endif
                                        </div>
                                    </div>
                                ...
                                <div id="pre-trial-collapse_{{ $loop->iteration }}" class="collapse {{ $application->latestApplicationRound->round->isTrialRound() ? '' : ($loop->last ? 'show' : '') }}">
                                        ...
                                        <div class="card-body">
                                            <div class="form-row">
                                                ...
                                                <div class="form-group offset-md-1 col-md-5">
                                                    <label class="text-secondary fz-14 leading-none mb-0.16">Applied
                                                        for</label>
                                                    <div>
                                                ...
                                                <div class="form-group offset-md-1 col-md-5">
                                                    <label class="text-secondary fz-14 leading-none mb-0.16">Graduation
                                                        Year</label>
                                                    <div>
                                    ...
                                    <div class="card-footer">
                                        <div class="d-flex align-items-center">
                                            @if (!$application->latestApplicationRound->round->isTrialRound())
                                            @if ($applicationRound->showActions)
                                            <select name="action_type" id="action_type" class="form-control w-50p" v-on:change="onSelectNextRound($event)" data-application-job-rounds="{{ json_encode($application->job->exceptTrialRounds) }}">
                                                <option v-for="round in applicationJobRounds" value="round" :data-next-round-id="round.id">
                                                    Move to @{{ round.name }}</option>
                                                <option value="send-for-approval">Send for approval
                                                </option>
                ...
                <div class="card">
                    <div class="card-header d-flex align-items-center justify-content-between">
                        ...
                        <div class="d-flex align-items-center">
                            ...
                            @endif
                            <div class="icon-pencil position-relative ml-3 c-pointer" data-toggle="collapse" data-target="#collapse_{{ $loop->iteration }}"><i class="fa fa-pencil"></i>
                            </div>
                        </div>
                    </div>
                    ...
                    <form action="/hr/recruitment/applications/rounds/{{ $applicationRound->id }}" method="POST" enctype="multipart/form-data" class="applicant-round-form">
                        ...
                        <div id="collapse_{{ $loop->iteration }}" class="collapse {{ $loop->last ? 'show' : '' }}">
                            <div class="card-body">
                                ...
                                <div class="card mb-2 border-0">
                                    ...
                                    <div class="card-body">
                                        <div class="form-row">
                                            ...
                                            <div class="form-group col-md-5">
                                                <label class="text-secondary fz-14 leading-none mb-0.16">Resume
                                                    Link</label>
                                                <div>
                                                    <a href="{{ route('desired.resume', [str_slug($application->job->title), $application->job->id]) }}" target="_blank">Sample Desired Resume
                                                    </a>
                                                </div>
                                            </div>
                                            @endif
                                ...
                                <div class="form-row d-flex justify-content-end">
                                    <button type="button" class="btn btn-info btn-sm round-submit" data-action="update">Update
                                        feedback</button>
                                </div>
                            </div>
                            @php
                            ...
                            <div class="card-footer">
                                <div class="d-flex align-items-center">
                                    @if ($applicationRound->showActions)
                                    <select name="action_type" id="action_type" class="form-control w-42p" v-on:change="onSelectNextRound($event)" data-application-job-rounds="{{ json_encode($application->job->exceptTrialRounds) }}">
                                        <option v-for="round in applicationJobRounds" value="round" :data-next-round-id="round.id">Move to
                                            @{{ round.name }}</option>
                                        <option value="send-for-approval">Send for approval
                                        </option>
                                        <option value="approve">Approve</option>
                                    ...
                                    <button type="button" class="btn btn-success ml-2" @click="takeAction()" id="takeAction">Take
                                        action</button>
