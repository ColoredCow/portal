    ...
    <div :class="[showResumeFrame ? 'container-fluid' : 'container']" id="page_hr_applicant_edit">
        <div class="container d-flex mt-5">
            ...
            <div class="card">
                            ...
                            <div class="mb-1">
                                @if ($application->latestApplicationRound->hr_round_id == 1)
                                    <a href="{{ route('applications.job.edit', $application->id) }}"
                                        class="btn btn-primary btn-sm" target="_self">Evaluate</a>
                                @endif
                                <button type="button" class="btn btn-primary btn-sm" data-toggle="modal"
                                    data-target="#customApplicationMail">Send mail</button>
                                @include('hr.custom-application-mail-modal', [
                                    'application' => $application,
                                ])
                            </div>
                            <div class="text-right fz-14">
                                <span class="c-pointer btn-clipboard text-right"
                                    data-clipboard-text="{{ $application->getScheduleInterviewLink() }}"
                    ...
                    <div class="form-row">
                        <div class="form-group col-md-5">
                            ...
                            <div>
                                ...
                                @if ($applicant->linkedin)
                                    <a href="{{ $applicant->linkedin }}" target="_blank"><i
                        ...
                        <div class="form-group offset-md-1 col-md-5">
                            ...
                            <div>
                                ...
                                @includeWhen(isset($hasGraduated) && !$hasGraduated,
                                    'hr.job-to-internship-modal',
                                    ['application' => $application]
                                )
                            </div>
                        </div>
                ...
                <form action="/hr/recruitment/applications/rounds/{{ $applicationRound['0']->id }}" method="POST"
                    enctype="multipart/form-data" class="applicant-round-form">
                    {{ csrf_field() }}
                    {{ method_field('PATCH') }}
                            ...
                            @if (!isset($applicationRound['0']->round_status))
                                <div class="form-row">
                                            ...
                                            <div>
                                                ...
                                                @if ($applicationRound['0']->scheduled_date)
                                                    @if ($applicationRound['0']->hangout_link)
                                                        <a target="_blank" class="ml-5 font-muli-bold"
                                                            href="{{ $applicationRound['0']->hangout_link }}">
                                    ...
                                    <div class="form-group col-md-4">
                                        ...
                                        @if ($applicationRound['0']->scheduled_date)
                                            <select name="scheduled_person_id" id="scheduled_person_id"
                                                class="form-control form-control-sm">
                                        ...
                                        @else
                                            <div class="fz-16 leading-tight">
                                                <img src="{{ $applicationRound['0']->scheduledPerson->avatar }}"
                                                    alt="{{ $applicationRound['0']->scheduledPerson->name }}"
                                    ...
                                    @if ($applicationRound['0']->scheduled_date)
                                        <div class="form-group col-md-3 d-flex align-items-end">
                                            <button type="submit"
                                                ...
                                                data-action="schedule-update">Update Schedule</button>
                                        </div>
                                    @endif
                                </div>
                            @endif
