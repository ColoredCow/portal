
@extends('layouts.app')

@section('content')
    <div class="container" id="home_page">
        @include('status', ['errors' => $errors->all()])
        <br>

        <div class="d-flex justify-content-start row flex-wrap">
            @can('hr_settings.view')
                <div class="col-md-4">
                    <div class="card h-75 mx-4 mt-3 mb-5 ">
                        <a class="card-body no-transition" href="{{ route('settings.hr') }}">
                            <br>
                            <h2 class="text-center">HR Mail Templates</h2><br>
                        </a>
                    </div>
                </div>
            @endcanany

            @can('nda_settings.view')
                <div class="col-md-4">
                    <div class="card h-75 mx-4 mt-3 mb-5 ">
                        <a class="card-body no-transition" href="{{ route('setting.agreement.nda') }}">
                            <br>
                            <h2 class="text-center">NDA Mail Templates</h2><br>
                        </a>
                    </div>
                </div>
            @endcanany

            @can('finance_invoices_settings.view')
                <div class="col-md-4">
                    <div class="card h-75 mx-4 mt-3 mb-5 ">
                        <a class="card-body no-transition" href="{{ route('setting.invoice') }}">
                            <br>
                            <h2 class="text-center">Invoice Mail Templates</h2><br>
                        </a>
                    </div>
                </div>
            @endcan
            @can('employee_salary_settings.view')
                <div class="col-md-4">
                    <div class="card h-75 mx-4 mt-3 mb-5 ">
                        <a class="card-body no-transition" href="{{ route('salary.settings') }}">
                            <br>
                            <h2 class="text-center">Salary</h2><br>
                        </a>
                    </div>
                </div>
            @endcan
            <div class="col-md-4">
                <div class="card h-75 mx-4 mt-3 mb-5 ">
                    <a class="card-body no-transition" href="{{ route('settings.bank-details') }}">
                        <br>
                        <h2 class="text-center">Bank Details</h2><br>
                    </a>
                </div>
            </div>
            <div class="col-md-4">
                <div class="card h-75 mx-4 mt-3 mb-5 ">
                    <a class="card-body no-transition" href="{{ route('settings.country-details') }}">
                        <br>
                        <h2 class="text-center">Country Details</h2><br>
                    </a>
                </div>
            </div>
            <div class="col-md-4">
                <div class="card h-75 mx-4 mt-3 mb-5 ">
                    <a class="card-body no-transition" href="{{ route('settings.config-variables') }}">
                        <br>
                        <h2 class="text-center">Config Variable</h2><br>
                    </a>
                </div>
            </div>
            <div class="col-md-4">
                <div class="card h-75 mx-4 mt-3 mb-5 ">
                    <a class="card-body no-transition" href="{{ route('settings.openai-instruction') }}">
                        <br>
                        <h2 class="text-center">OpenAI Instruction</h2><br>
                    </a>
                </div>
            </div>
        </div>
    </div>
@endsection