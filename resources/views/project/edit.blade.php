                        ...
                        <div class="input-group">
                            <div class="input-group-prepend">
                                ...
                                @foreach (config('constants.currency') as $currency => $currencyMeta)
                                    @php
                                        $selected = $currency === $project->currency_cost ? 'selected="selected"' : '';
                                    @endphp
                                    <option value="{{ $currency }}" {{ $selected }}>{{ $currency }}</option>
                                @endforeach
                                </select>
                            </div>
                            <input type="number" class="form-control" name="cost" id="cost" placeholder="Project cost" step=".01" min="0" value="{{ $project->cost }}">
                        </div>
                    </div>
                </div>
            </div>
