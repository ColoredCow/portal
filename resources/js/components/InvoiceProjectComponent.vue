    ...
    <div class="form-row row d-flex align-items-center">
        ...
        <div class="form-group col-md-3">
            ...
            <select class="form-control" v-model="stageId" v-on:change="updateActiveStage()">
                <option v-for="stage in activeProject.stages" :value="stage.id" :key="stage.id">{{ stage.name }}</option>
            </select>
        </div>
        <div class="form-group col-md-2">
            <label>Billing</label>
            <select name="billings[]" class="form-control" v-model="billingId">
                <option v-for="stageBilling in activeStage.billings" :value="stageBilling.id" :key="stageBilling.id">{{ stageBilling.percentage + '%' }} –  {{ client.currency }} {{ stageBilling.amount }}</option>
            </select>
        </div>
        <div class="col-md-1">
