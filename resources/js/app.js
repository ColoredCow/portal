...
if (document.getElementById("finance_report")) {
	const financeReport = new Vue({
		...
		data: {
			...
			sentAmountINR:
				document.getElementById("sent_amount_INR").dataset.sentAmount ||
				0,
			sentAmountUSD:
				document.getElementById("sent_amount_USD").dataset.sentAmount ||
				0,
			conversionRateUSD:
...
if (document.getElementById("show_and_save_book")) {
	const bookForm = new Vue({
		...
		data: {
			...
			routes: {
				index:
					document.getElementById("show_book").dataset.indexRoute ||
					"",
				fetch:
					document.getElementById("book_form").dataset.actionRoute ||
					"",
				store:
					document.getElementById("show_book").dataset.storeRoute ||
					"",
			},
			buttons: {
...
if (document.getElementById("books_listing")) {
	const bookForm = new Vue({
		...
		data: {
			...
			categoryIndexRoute:
				document.getElementById("books_table").dataset
					.categoryIndexRoute || "",
			categoryInputs: [],
			currentBookIndex: 0,
			newCategory: "",
	...
	const bookForm = new Vue({
		...
		methods: {
			showEditMode: function(index) {
				this.categoryNameToChange[index] = this.categories[index][
					"name"
				];
	...
	var userRoles = new Vue({
		...
		data: {
			...
			roles: document.getElementById("user_roles_table").dataset.roles
				...
				: "",
			updateRoute:
				document.getElementById("user_roles_table").dataset
					.updateRoute || "",
			currentUserIndex: 0,
			roleInputs: [],
		},
