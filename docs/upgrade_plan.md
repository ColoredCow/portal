...
> - **Factory work is smaller than stated:** only the **6** root `database/factories` files use old-style `$factory->define()`; **all module factories are already class-based** (so "~16 in modules / 22 total" no longer applies). Of the 6, only **3 are real conversions** (`BookFactory`, `ClientFactory`, `SettingFactory`); the other 3 (`ApplicantFactory`, `JobFactory`, `RoundFactory`) reference `App\Models\HR\*` classes that no longer exist (models moved to `Modules/HR/Entities`) — **delete those, don't migrate them**.
...
4. **Migrate factories to class-based** and drop `laravel/legacy-factories`. Of the **6** old-style `$factory->define()` factories in `database/factories`, **convert 3** whose models exist (`BookFactory`, `ClientFactory`, `SettingFactory`) and **delete 3** that are dead code (`ApplicantFactory`, `JobFactory`, `RoundFactory` reference `App\Models\HR\{Applicant,Job,Round}`, which moved to `Modules/HR/Entities` — nothing else references them). **The module factories are already class-based** (verified 2026-06-09), so the original "~16 in modules / 22 total" no longer applies. Preserve `faker_locale => en_IN`. (Class factories work on Laravel 8, so this ships now.)
...
- [ ] Root factories resolved — 3 converted to class-based (`Book`, `Client`, `Setting`), 3 dead ones deleted (`Applicant`, `Job`, `Round`); `laravel/legacy-factories` removed (module factories already class-based).
...
- 6 old-style factories in `database/factories` (3 convertible; 3 dead — they reference removed `App\Models\HR\*` models); all module factories already class-based (corrected 2026-06-09 — original "22 / ~16 in modules" was inaccurate).
