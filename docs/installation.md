...
14. _(Optional)_ Setup API Layer API Key for currency excahange rates:
    ...
    6. Copy your API key and paste it in the `.env` file.
       `CURRENCYLAYER_API_KEY=your api key`

    7. Run the command `php artisan config:cache`
...
15. _(Optional)_ Setup Google Service Account for syncing effortsheet on local:
    ...
    12. Copy the Email from the Service Accounts details
    ...
    13. Add the email by clicking on the share button
    ...
    14. Run the command php artisan config:cache

    15. Open the ColoredCow portal http://portal.test/login/

    16. Go to CRM->Projects and select any of the projects.

    17. After opening the project click on edit button and enter the Effortsheet URL then, click on Save button.
