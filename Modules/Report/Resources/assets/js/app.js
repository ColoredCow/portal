$(function() {
  ...
  if ($("#userDashboardGraph").length) {
    getData({ type: "fte-trend" }, userFteTrendsReport);
  }
});
...
function financeReportRevenueTrendsReport(reportsData) {
  ...
  const currentPeriodTotalRevenue =
    reportsData.data.current_period_total_amount;
  const previousPeriodTotalRevenue =
    reportsData.data.previous_period_total_amount;
  const chartData = {
    labels: labels,
    datasets: [
      {
