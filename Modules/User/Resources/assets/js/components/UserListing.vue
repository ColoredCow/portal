<template>
	<div>
		...
		<table class="table table-bordered table-striped">
			...
			<tbody>
				<tr v-for="(user, index) in filteredUsers" :key="index">
					...
					<td>
						<span>{{ formatRoles(user)
						}}<span v-if="user.websiteUserRole">, {{ user.websiteUserRole }}</span></span>
					</td>
					<td>
						<div class="dropdown d-none">
							...
							<div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
								<a v-show="userPermissions['can-assign-roles']" class="dropdown-item" data-toggle="modal"
									data-target="#update_user_roles_modal" @click="updateUserRolesModal(index)"
									href="#">Assign roles</a>
								<a v-show="userPermissions['can-delete']" class="dropdown-item text-danger"
									@click="removeUser(index)" ref="#">Remove this user</a>
							</div>
						</div>
						<div>
							<button v-show="userPermissions['can-assign-roles']" class="btn btn-sm btn-outline-info mr-4"
								data-toggle="modal" data-target="#update_user_roles_modal"
								@click="updateUserRolesModal(index)">
								Manage user roles
							</button>
				...
				<div class="modal-content">
					<div class="modal-header">
						<h5 class="modal-title">
							Delete user: <strong> {{ users[currentUserIndex].name }}</strong>
						</h5>
					</div>
					...
					<div class="modal-footer">
						...
						<button type="button" @click="removeUser(index)" class="btn btn-danger">
							Yes
						</button>
					</div>
				</div>
		...
		<user-role-update-modal :user="this.selectedUser" :updateRoute="this.updateRoute" :config="config"
			@userRolesUpdated="this.onUserRoleUpdated" />
		<user-staff-type-update-modal :user="this.selectedUser" :stafftypes="stafftypes" />
	</div>
</template>
<script>
export default {
