    ...
    <table class="table table-bordered table-striped">
        ...
        <tbody>
            <tr v-for="(role, index) in allRoles" :key="index">
                ...
                <td class="w-300">
                    <button class="btn btn-sm btn-outline-info" @click="updatePermissionModal(index)" data-toggle="modal" data-target="#update_role_permissions_modal">
                        Manage permissions
                    </button>
                    <button type="button" data-toggle="modal" v-on:click="deleteRole(index)" class="btn btn-sm btn-outline-danger">
