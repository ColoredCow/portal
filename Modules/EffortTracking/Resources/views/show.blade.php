                    ...
                    <h4>{{ $project->name }} - Members
                        ...
                        <i class="ml-2 font-weight-bold fa fa-refresh c-pointer" aria-hidden="true"
                            ...
                            <div class="fz-14 float-right">&nbsp;&nbsp;&nbsp;
                                <strong>Last refreshed at: </strong> {{config('project.meta_keys.project.key') . __(''). (Carbon\Carbon::parse($project->last_updated_at)->setTimezone('Asia/Kolkata')->format(' Y-M-d , D h:i:s A'))}}
                            </div > 
                            <div class="fz-14 float-right">
                               <strong>Timeline:</strong> {{ config('project.meta_keys.project.key') . __(''). (Carbon\Carbon::parse($project->client->month_start_date )->format('dS M'))}}                       
                                {{ config('project.meta_keys.project.key') . __(' - ') . (Carbon\Carbon::parse($project->client->month_end_date)->format('dS M'))}}              
                            </div>
                    </h4>
                </div>
            </div>
