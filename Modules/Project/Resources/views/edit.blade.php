...
<div class="container" id="view_edit_project">
    <br> <h4 clss="c-pointer" v-on:click="counter += 1">{{ $project->name }} ({{ $project->client_project_id }})<a id="view_effort_sheet_badge" class="badge badge-primary p-1 ml-2 text-light pl-3 pr-3 {{ $project->effort_sheet_url ? '' : 'd-none' }}" target="blank" href="{{route('project.effort-tracking', $project )}}">{{ _('FTE') }}</a>
    </h4>
    <br>
