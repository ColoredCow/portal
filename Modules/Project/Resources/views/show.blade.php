...
@section('content')
    <div class="efforts-sync-loader text-center bg-theme-semi-transparent position-fixed w-full h-full d-none"
        style="top: 0%; right: 0%; z-index: 9999" id="preloader">
        <div class="spinner-border position-relative"
            style="top: 40%; border: 0.25em solid #ffff; border-right-color: transparent;" id="spinner">
    ...
    <div class="container" id="vueContainer">
        <br>
        <div class=" d-flex">
            <h4 class="c-pointer d-inline-block font-weight-bold" v-on:click="counter += 1">Project Name :
                {{ $project->name }}</h4>
            @can('update', $project)
                ...
                <div class="container" id="vueContainer">
                    <div class="form-row">
                        <div class="form-group col-md-6 pl-4">
                            ...
                            <span
                                class="{{ $project->velocity >= 1 ? 'text-success' : 'text-danger' }} fz-lg-22">{{ $project->velocity }}</span>
                        ...
                        <div class="form-group col-md-6 pl-10">
                            ...
                            @if ($project->effort_sheet_url)
                                <a id="view_effort_sheet_badge" href="{{ $project->effort_sheet_url }}"
                                    class="btn-sm btn-primary btn-smtext-white ml-2 text-light rounded"
                    ...
                    <div class="form-row">
                        <div class="form-group col-lg-12 pl-4">
                            <div class="fz-14 mr-3 mt-1 d-flex justify-content-between">
                                <h4 class="d-inline-block ">
                                    <label for="name" class="font-weight-bold">Team
                                        Members({{ count($project->getTeamMembers) }})</label>
                                </h4>
                                <div class="d-flex">
                                    <h4 class="mr-5">
                                        <i class="fa fa-spinner fa-spin ml-2 d-none"></i>
                                        <i class="ml-2 font-weight-bold fa fa-refresh c-pointer" aria-hidden="true"
                                            @if (auth()->user()->can('finance_invoices.create')) data-toggle="modal" data-target="#syncEffortsModal"
                                    @else
                                        data-url="{{ route('effort-tracking.refresh', $project) }}" @endif>
                                        </i>
                                    </h4>
                                        ...
                                        @foreach ($project->getTeamMembers as $teamMember)
                                            <tr>
                                                <th class="fz-lg-20 my-2 px-5 font-weight-normal">
                                                    <span>
                                                        <span class="tooltip-wrapper" data-html="true" data-toggle="tooltip"
                                                            title="{{ $teamMember->user->name }} - {{ config('project.designation')[$teamMember->designation] }}">
                                                            <a
                                                                href="{{ route('employees.show', optional($teamMember->user->employee)->id) }}">
                                                                <img src="{{ $teamMember->user->avatar }}"
                                                                    class="w-35 h-30 rounded-circle mr-1 mb-1">
                                                            </a>
                                                        </span>
                                                        <a
                                                            href="{{ route('employees.show', optional($teamMember->user->employee)->id) }}">
                                                ...
                                                <td data-toggle="tooltip"
                                                    title="Start date: {{ $teamMember->started_on->format('Y-m-d') }}  {{ $teamMember->ended_on != null ? 'End date: ' . $teamMember->ended_on->format('Y-m-d') : '' }}">
                                                    {{ $effortData['totalWorkingDays'] }} Days |
                                                    {{ $effortData['daysTillToday'] }} Days</td>
                            ...
                            @if ($isApprovedWorkPipelineExist)
                                <span>{{ $monthlyApprovedHour }} hrs
                                    @if ($monthlyApprovedHour > $weeklyHoursToCover)
                                        <span class="tooltip-wrapper" data-html="true" data-toggle="tooltip"
                                            title="Approved pipeline is sufficient for this week"><i
                                                class="fa fa-info-circle" style="color: green;"
                                                aria-hidden="true"></i></span>
                                    @else
                                        <span class="tooltip-wrapper" data-html="true" data-toggle="tooltip"
                                            title="Approved pipeline is not sufficient for this week"><i
                                                class="fa fa-info-circle" style='color: red'
                                                aria-hidden="true"></i></span>
                                    @endif
                                </span>
                            @else
                                <span class="text-capitalize ml-2 text-danger fz-18 font-weight-normal">
                                    ERROR <span class="tooltip-wrapper" data-html="true" data-toggle="tooltip"
                                        title="Formatting issue with effortsheet"><i class="fa fa-info-circle"
                                            aria-hidden="true"></i></span>
                                </span>
                            @endif
                        </h4>
            ...
            <div class="m-5">
                <div class="d-flex" style="flex-wrap: wrap">
                    <div class="pr-10 project-hour w-33p mb-10">
                        ...
                        <span
                            class="text-capitalize  fz-lg-22">{{ optional($project->start_date)->format('d M Y') }}</span>
                    </div>
                    <div class="pr-10 project-hour w-33p mb-10">
                        <h4 class="d-inline-block">
                    ...
                    <div class="pr-10 project-hour w-33p mb-10">
                        ...
                        <a href="{{ route('client.edit', $project->client->id) }}"
                            class="text-capitalize fz-lg-22">{{ $project->client->name }}</a>
                    </div>
                    <div class="pr-10 project-hour w-33p mb-10">
                        <h4 class="d-inline-block">
                    ...
                    <div class="pr-10 project-hour w-33p mb-10">
                        ...
                        <div class="flex-row">
                            @if ($project->projectContracts->isEmpty() == false)
                                <a id="contract_file" style="{{ $project->projectContracts ? '' : 'd-none' }}"
                                    href="{{ route('pdf.show', $project->projectContracts->first()) }}">
                                    <span
                                        class="text-capitalize fz-lg-22">{{ $project->name }}_contract_{{ optional($project->start_date)->format('d M Y') }}</span>
        ...
        new Vue({
            ...
            methods: {
                ...
                calculateDelay(endDate, expectedEndDate) {
                    const delayDays = Math.floor((new Date(endDate).setHours(0, 0, 0, 0) - new Date(expectedEndDate)
                ...
                submitForm() {
                    ...
                    const updatedStages = this.stages.filter(stage => stage.id && stage.isUpdated).map(this
                        .cleanStageData);
                ...
                checkOverdueStatus(stage) {
                    if (new Date(stage.expected_end_date) < new Date(this.currentDate) && stage.status !==
                        'completed') {
