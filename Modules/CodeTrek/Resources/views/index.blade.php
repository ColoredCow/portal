                ...
                <form action="{{ route('codetrek.index') }}" id="centreFilterForm">
                    <div class="form-group ml-25 w-180">
                        @can('codetrek_applicant.create')
                            <select class="form-control bg-light" name="centre" id="centre"
                                onchange="document.getElementById('centreFilterForm').submit();">
                                <option value=""
                                    {{ !request()->has('centre') || empty(request()->get('centre')) ? 'selected' : '' }}>
                                    {!! __('All Centres') !!}
                                </option>
                                @foreach ($centres as $centre)
                                    <option value="{{ $centre->id }}"
                                        {{ request()->get('centre') == $centre->id ? 'selected' : '' }}>
                                        {{ $centre->centre_name }}
                                    </option>
                                @endforeach
                            </select>
        ...
        <ul class="nav nav-pills d-flex justify-content-between">
            ...
            @elseif (request()->tab == 'reports')
                <ul class="nav nav-tabs">
                    <li class="nav-item mr-3">
                        <a class="nav-link active" href="#daily-tab" data-toggle="tab">Daily Applications</a>
                    </li>
                    <li class="nav-item mr-3" id="monthly_tab_nav_item"
                        data-route="{{ route('reports.codetrek.get-report-data-monthly') }}">
                        <a class="nav-link" href="#monthly-tab" data-toggle="tab">Monthly Applications</a>
                    </li>
                </ul>
            @endif
            <form action="{{ route('codetrek.index') }}" id="sortingForm">
                <div class="form-group w-120">
                    <select class="form-control bg-light" name="order" id="order"
                        ...
                        <tr class="text-center sticky-top">
                            ...
                            <th>
                                <span class="dropdown-toggle c-pointer" data-toggle="dropdown" aria-haspopup="true"
                                    aria-expanded="false" id="statusDropdown">Level</span>
                        ...
                        @forelse ($applicants as $applicant)
                            <tr>
                                <td>
                                    ...
                                    @if ($applicant->phone)
                                        <span class="mr-1"><i
                                                class="fa fa-phone mr-1"></i>{{ $applicant->phone }}</span>
                                        <br>
                                    @endif
                                    <div>
                                ...
                                <td>
                                    <div class="d-flex justify-content-center">
                                        <span
                                            class="{{ config('codetrek.rounds.' . $applicant->latest_round_name . '.class') }} badge-pill mr-1 mb-1 fz-16">
                                ...
                                <td>
                                    <div class="d-flex align-items-center">
                                        @if ($applicant->mentor)
                                            <div class="col">
                                                <div class="d-flex align-items-center">
                                                    <img src="{{ $applicant->mentor->avatar }}"
                                                        class="w-35 h-30 rounded-circle ">
                ...
                <div class="tab-pane active" id="daily-tab">
                    <div>
                        ...
                        <div class="card-header mt-1" align="left">
                            <div class="my-2">
                                <form align="right" action="{{ route('codetrek.index', ['tab' => 'reports']) }}"
                                    method="GET">
                                    <input type="date" name="application_start_date" id="start_date"
                                        value="{{ old('application_start_date', request()->get('application_start_date')) }}"
                                        required> to
                                    <input type="date" name="application_end_date" id="end_date"
                                        value="{{ old('application_end_date', request()->get('application_end_date')) }}"
                                        required>
                ...
                <div class="tab-pane" id="monthly-tab">
                    <div>
                        ...
                        <div class="card-header mt-1" align="left">
                            ...
                            <canvas class="w-full" id="CodeTrekApplicationReportMonthly"></canvas>
                        </div>
                    </div>
                </div>
            </div>
