<tr>
        ...
        <div class="mb-2 fz-xl-14 text-secondary d-flex flex-column">
            ...
            <div>
                @if ($application->applicant->college && $application->applicant->university)
                    <span class="mr-1"><i
                            class="fa fa-university mr-1"></i>{{ $application->applicant->college }}</span>
                    <a href="{{ route('universities.edit', $application->applicant->university) }}" target="_blank">
                        @if ($application->applicant->university->universityContacts->first() &&
                            $application->applicant->university->universityContacts->first()->phone)
    ...
    <td>
        <div class="d-flex flex-column">
            <span>{{ $application->job->title }} <span data-toggle="tooltip" data-placement="right"
                    title="Total resources required -{{ $application->job->resources_required }} Total applications available -{{ $openApplicationsCountForJobs[$application->job->title] }}"><i
