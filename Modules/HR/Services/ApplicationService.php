...
class ApplicationService implements ApplicationServiceContract {
	...
	public function index( $applicationType ) {
		...
		$countFilters   = array_except( $filters, array( 'status', 'round' ) );
		$attr           = array(
			'applications' => $applications,
			'status'       => request()->get( 'status' ),
		);
		$hrRounds       = array( 'Resume Screening', 'Telephonic Interview', 'Introductory Call', 'Basic Technical Round', 'Detailed Technical Round', 'Team Interaction Round', 'HR Round', 'Trial Program', 'Volunteer Screening' );
	...
	public function addSubscriberToCampaigns( $parameters, $subscriptionLists ) {
		...
		$subscriptionLists = is_array( $subscriptionLists ) ? $subscriptionLists : array( $subscriptionLists );
	...
	public function getApplicationsForDate( $today, $selectedDate, $searchCategory, $selectedJob, $selectedOpportunity, $selectedRound ) {
		...
		// Additional filters
		if ( $selectedJob !== null ) {
			$interviewApplicationsQuery->whereHas(
				'application.job',
				function ( $query ) use ( $selectedJob ) {
		...
		if ( $selectedOpportunity !== null ) {
			$interviewApplicationsQuery->whereHas(
				'application.job',
				function ( $query ) use ( $selectedOpportunity ) {
		...
		if ( $searchCategory !== null ) {
			$interviewApplicationsQuery->where(
		...
		// Process interview applications
