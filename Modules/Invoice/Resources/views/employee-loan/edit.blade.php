...
@section('content')
    <div class="container">
        ...
        @include('back-to-previous-page', [
            'url' => route('employee-loan.index'),
            'text' => 'Back to Loan List',
        ])
                ...
                <div class="card-body">
                    <div class="form-row">
                        <div class="form-group col-md-5">
                            ...
                            <input class="form-control"
                                value="{{ optional($employeeLoan->employee->user)->name ?? 'User Deleted' }}" disabled>
                        </div>
                        <div class="form-group offset-md-1 col-md-5">
                            ...
                            <input type="number" class="form-control" name="total_amount" placeholder="Enter Loan Amount"
                                required="required" step=".01" min="0"
                                value="{{ old('total_amount') ?: $employeeLoan->total_amount }}">
                        </div>
                    </div>
                    <div class="form-row">
                    ...
                    <div class="form-row">
                        <div class="form-group col-md-5">
                            ...
                            <input type="month" class="form-control" name="start_date" required="required"
                                value="{{ $employeeLoan->start_date->format('Y-m') }}">
                        </div>
                        <div class="form-group offset-md-1 col-md-5">
                            <label class="field-required">Loan End Month</label>
                            <input type="month" class="form-control" name="end_date" required="required"
                                value="{{ $employeeLoan->end_date->format('Y-m') }}">
                        </div>
                    </div>
                    <div class="form-row">
                        <div class="form-group col-md-5">
                            ...
                            <input type="number" class="form-control" name="monthly_deduction" placeholder="Enter Amount"
                                required="required" step=".01" min="0"
                                value="{{ old('monthly_deduction') ?: $employeeLoan->monthly_deduction }}">
                        </div>
                        <div class="form-group offset-md-1 col-md-5">
                            <label class="field-required">Loan End Month</label>
                            <select class="form-control" name="status" required="required">
                                @foreach (config('invoice.loan.status') as $status)
                                    <option value="{{ $status['slug'] }}"
                                        {{ $employeeLoan->status === $status['slug'] ? 'selected' : '' }}>
                                        {{ $status['label'] }} </option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <tbody>
                        ...
                        @foreach ($installments as $index => $installment)
                            ...
                            <tr>
                                ...
                                <td><i class="fa fa-check-circle text-success fz-20">{{ __(' Deducted from Salary') }}</i>
                                </td>
                            </tr>
                        @endforeach
                        @while ($remainingAmount > 0)
                            ...
                            <tr>
                                ...
                                <td>{{ $remainingAmount < 0 ? $employeeLoan->monthly_deduction + $remainingAmount : $employeeLoan->monthly_deduction }}
                                </td>
                                <td>{{ $remainingAmount < 0 ? 0 : $remainingAmount }}</td>
                                <td><i class="fa fa-calendar text-theme-orange fz-20">{{ __(' Future Installment') }}</i>
                                </td>
                            </tr>
                        @endWhile
                    </tbody>
