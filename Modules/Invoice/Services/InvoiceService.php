    ...
    private function formatMonthlyInvoicesForExportAll($invoices)
    {
        return $invoices->map(function ($invoice) {
            return [
                ...
                'INVOICE VALUE' => str_replace(['$', '₹'], '', $invoice->invoiceAmount()),
                'RATE' => '',
                'RECEIVABLE AMOUNT' => ClientAddress::select('*')->where('client_id', $invoice->client_id)->first() ? str_replace(['$', '₹'], '', $invoice->invoiceAmount()) : '',
                'TAXABLE AMOUNT' => str_replace(['$', '₹'], '', $invoice->display_amount),
