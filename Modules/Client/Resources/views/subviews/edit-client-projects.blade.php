    ...
    <div id="client_projects_container" class="collapse show">
        <div class="card-body">
            ...
            @foreach ($projects as $project)
                ...
                <div class="w-30p ml-2">
                    <h5 class="font-muli-bold">Day</h5>
                </div>
                <div class="w-20p ml-2">
                    <h5 class="font-muli-bold">Actions</h5>
                </div>
            ...
            <div class="d-flex px-5 my-5">
                <div class="w-33p ml-2">
                    @if (optional($client->project_review)->user)
                        <span class="tooltip-wrapper" data-html="true" data-toggle="tooltip" title="{{ $client->project_review->user->name  }}">
                            <img src="{{ $client->project_review->user->avatar }}" class="w-35 h-30 rounded-circle mr-1 mb-1">
                        </span>
                    @else
                        NA
                    @endif
                ...
                <div class="w-40p w-md-40p ml-2">
                @if (optional($client->project_review)->next_review_date)
                    <p>{{ $client->project_review->next_review_date }}</p>
                @else
                    NA
                @endif
                </div>
                <div class="w-30p ml-2">
                @if (optional($client->project_review)->meeting_day)
                    <p>{{ config('constants.working_week_days')[$client->project_review->meeting_day] }}</p>
                @else
                    NA
                @endif
                </div>
